                ...
                <div key={order._id}>
                  <div className="lg:flex context gap-x-16  text-lg font-[500] text-text-secondary">
                    <div className="lg:w-[50%]">
                      <div className="">
                        <p className="my-2 text-[1.4rem]">Order #{order._id}</p>
                        <p className="my-2">{new Date(order.date).toLocaleDateString("en-US", options)}</p>
                        <p className="my-2">Shipping to : {order.shipping_address.city}, {order.shipping_address.state}</p>
                        <p className="my-2">{order.first_name} {order.last_name}, {order.phone}</p>
                        <p className="my-2 leading-[1.2rem]">
                          {order.shipping_address.address_line1} <br />
                          {order.shipping_address.address_line2} <br />
                          {order.shipping_address.city} {order.shipping_address.pincode} {order.shipping_address.state} {order.shipping_address.country}
                        </p>
                      </div>
                    ...
                    <div className="lg:w-[50%] ">
                      <div>
                        {order.cart.map((item) => (
                          <p key={item._id} className="my-2 leading-[1.2rem]">
