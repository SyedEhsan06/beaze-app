...
export default function Contentcategories({ params }) {
  ...
  let colors = data?.products?.flatMap((item) => item.colors); // Use flatMap to flatten the array
  let uniqueColors = [...new Set(colors)];

  let sizes = data?.products?.flatMap((item) => item.sizes);
  let uniqueSizes = [...new Set(sizes)];

  let materials = data?.products?.flatMap((item) => item.material);
  let uniqueMaterials = [...new Set(materials)];

  let filterData = [
    {
      title: "Color",
      subcategory: uniqueColors.map((item) => {
        return { title: item };
      }),
    },
    {
      title: "Size",
      subcategory: uniqueSizes.map((item) => {
        return { title: item };
      }),
    },
    {
      title: "Material",
      subcategory: uniqueMaterials.map((item) => {
        return { title: item };
      }),
    },
  ];
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={`your-specific-class fixed overflow-y-auto right-0 h-[100vh] bg-white shadow-sm w-[350px] p-4 top-0 z-30 rounded-tl-[28px] border py-3 px-4  context ${
